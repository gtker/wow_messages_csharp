...
public class CMD_REALM_LIST_Server: Version3ServerMessage, ILoginMessage {
    ...
    public async Task WriteAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        await w.WriteByte((byte)Realms.Count, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMD_REALM_LIST_Server> ReadAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        // ReSharper disable once UnusedVariable.Compiler
        var numberOfRealms = await r.ReadByte(cancellationToken).ConfigureAwait(false);

        var realms = new List<Realm>();
        for (var i = 0; i < numberOfRealms; ++i) {
            realms.Add(await Version3.Realm.ReadAsync(r, cancellationToken).ConfigureAwait(false));
        }
