...
public class Realm {
    public required Version3.RealmType RealmType { get; set; }
    public required Version3.RealmFlag Flag { get; set; }
    public required string Name { get; set; }
    public required string Address { get; set; }
    public required Population Population { get; set; }
    public required byte NumberOfCharactersOnRealm { get; set; }
    public required Version3.RealmCategory Category { get; set; }
    ...
    public static async Task<Realm> ReadAsync(Stream r, CancellationToken cancellationToken = default) {
        var realmType = (Version3.RealmType)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        var category = (Version3.RealmCategory)await r.ReadByte(cancellationToken).ConfigureAwait(false);
