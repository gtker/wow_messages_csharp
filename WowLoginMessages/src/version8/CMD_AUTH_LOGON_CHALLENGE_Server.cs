...
public class CMD_AUTH_LOGON_CHALLENGE_Server: Version8ServerMessage, ILoginMessage {
    ...
    public async Task WriteAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        if (Result.Value is CMD_AUTH_LOGON_CHALLENGE_Server.LoginResultSuccess result) {
            ...
            await w.WriteByte((byte)result.SecurityFlag.Inner, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMD_AUTH_LOGON_CHALLENGE_Server> ReadAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        if (result.Value is Version8.LoginResult.Success) {
            ...
            if (securityFlag.Inner.HasFlag(Version8.SecurityFlag.Authenticator)) {
                var required = await r.ReadByte(cancellationToken).ConfigureAwait(false);
