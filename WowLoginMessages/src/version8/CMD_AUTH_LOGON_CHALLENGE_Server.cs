...
public class CMD_AUTH_LOGON_CHALLENGE_Server: Version8ServerMessage, ILoginMessage {
    ...
    public async Task WriteAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        if (Result == LoginResult.Success) {
            ...
            if (SecurityFlag.HasFlag(SecurityFlag.Authenticator)) {
                await WriteUtils.WriteByte(w, Required, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMD_AUTH_LOGON_CHALLENGE_Server> ReadAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        if (result == LoginResult.Success) {
            ...
            if (securityFlag.HasFlag(SecurityFlag.Authenticator)) {
                required = await ReadUtils.ReadByte(r, cancellationToken).ConfigureAwait(false);
