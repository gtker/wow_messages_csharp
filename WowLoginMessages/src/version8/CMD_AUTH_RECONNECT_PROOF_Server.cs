...
public class CMD_AUTH_RECONNECT_PROOF_Server: Version8ServerMessage, ILoginMessage {
    public required Version8.LoginResult Result { get; set; }
    ...
    public static async Task<CMD_AUTH_RECONNECT_PROOF_Server> ReadAsync(Stream r, CancellationToken cancellationToken = default) {
        var result = (Version8.LoginResult)await r.ReadByte(cancellationToken).ConfigureAwait(false);
