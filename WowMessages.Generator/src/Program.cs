...
internal static class Program
{
    ...
    private static void WriteLoginFiles(IntermediateRepresentationSchema schema, byte version)
    {
        ...
        foreach (var e in schema.Login.Enums.Value)
        {
            ...
            File.WriteAllText(ProjectDir + $"WowMessages.{project}/src/{modulePath}/" + e.FileName(),
                s.ToString());
        }
        ...
        File.WriteAllText(ProjectDir + $"WowMessages.{project}Test/{module}.cs", tests.ToString());
        ...
        {
            File.WriteAllText(ProjectDir + $"WowMessages.{project}/src/{modulePath}/ServerOpcodeReader.cs",
                serverS.ToString());
        }
        ...
        {
            File.WriteAllText(ProjectDir + $"WowMessages.{project}/src/{modulePath}/ClientOpcodeReader.cs",
                clientS.ToString());
        }
    }
}