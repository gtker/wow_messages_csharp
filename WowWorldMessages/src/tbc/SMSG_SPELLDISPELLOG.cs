...
public class SMSG_SPELLDISPELLOG: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_SPELLDISPELLOG> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfSpells; ++i) {
            spells.Add(await Tbc.DispelledSpell.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
