...
public class SMSG_GUILD_ROSTER: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_GUILD_ROSTER> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfRights; ++i) {
            rights.Add(await Tbc.GuildRights.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }

        var members = new List<GuildMember>();
        for (var i = 0; i < amountOfMembers; ++i) {
            members.Add(await Tbc.GuildMember.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
