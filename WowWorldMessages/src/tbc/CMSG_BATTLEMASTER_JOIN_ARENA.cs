...
public class CMSG_BATTLEMASTER_JOIN_ARENA: TbcClientMessage, IWorldMessage {
    public required ulong Battlemaster { get; set; }
    public required Tbc.JoinArenaType ArenaType { get; set; }
    ...
    public static async Task<CMSG_BATTLEMASTER_JOIN_ARENA> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var arenaType = (Tbc.JoinArenaType)await r.ReadByte(cancellationToken).ConfigureAwait(false);
