...
public class SMSG_PET_TAME_FAILURE: TbcServerMessage, IWorldMessage {
    public required Tbc.PetTameFailureReason Reason { get; set; }
    ...
    public static async Task<SMSG_PET_TAME_FAILURE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var reason = (Tbc.PetTameFailureReason)await r.ReadByte(cancellationToken).ConfigureAwait(false);
