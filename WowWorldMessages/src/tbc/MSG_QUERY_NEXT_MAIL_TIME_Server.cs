...
public class MSG_QUERY_NEXT_MAIL_TIME_Server: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<MSG_QUERY_NEXT_MAIL_TIME_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfMails; ++i) {
            mails.Add(await Tbc.ReceivedMail.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
