...
public class SMSG_PARTY_COMMAND_RESULT: TbcServerMessage, IWorldMessage {
    public required Tbc.PartyOperation Operation { get; set; }
    public required string Member { get; set; }
    public required Tbc.PartyResult Result { get; set; }
    ...
    public static async Task<SMSG_PARTY_COMMAND_RESULT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var operation = (Tbc.PartyOperation)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var member = await r.ReadCString(cancellationToken).ConfigureAwait(false);

        var result = (Tbc.PartyResult)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
