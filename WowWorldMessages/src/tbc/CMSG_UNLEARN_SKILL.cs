...
public class CMSG_UNLEARN_SKILL: TbcClientMessage, IWorldMessage {
    public required Tbc.Skill Skill { get; set; }
    ...
    public static async Task<CMSG_UNLEARN_SKILL> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var skill = (Tbc.Skill)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
