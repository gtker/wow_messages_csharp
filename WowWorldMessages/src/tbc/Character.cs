...
public class Character {
    ...
    public required Tbc.CreatureFamily PetFamily { get; set; }
    public const int EquipmentLength = 20;
    ...
    public static async Task<Character> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var petFamily = (Tbc.CreatureFamily)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var equipment = new CharacterGear[EquipmentLength];
        for (var i = 0; i < EquipmentLength; ++i) {
            equipment[i] = await Tbc.CharacterGear.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
        }
