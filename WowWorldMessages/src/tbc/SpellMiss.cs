...
public class SpellMiss {
    ...
    public static async Task<SpellMiss> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        SpellMissInfoType missInfo = (Tbc.SpellMissInfo)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (missInfo.Value is Tbc.SpellMissInfo.Reflect) {
