...
public class SMSG_STANDSTATE_UPDATE: TbcServerMessage, IWorldMessage {
    public required Tbc.UnitStandState State { get; set; }
    ...
    public static async Task<SMSG_STANDSTATE_UPDATE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var state = (Tbc.UnitStandState)await r.ReadByte(cancellationToken).ConfigureAwait(false);
