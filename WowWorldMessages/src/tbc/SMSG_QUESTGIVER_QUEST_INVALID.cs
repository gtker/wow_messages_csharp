...
public class SMSG_QUESTGIVER_QUEST_INVALID: TbcServerMessage, IWorldMessage {
    public required Tbc.QuestFailedReason Msg { get; set; }
    ...
    public static async Task<SMSG_QUESTGIVER_QUEST_INVALID> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var msg = (Tbc.QuestFailedReason)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
