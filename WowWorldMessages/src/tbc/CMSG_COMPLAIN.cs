...
public class CMSG_COMPLAIN: TbcClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 966, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_COMPLAIN> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        SpamTypeType complaintType = (Tbc.SpamType)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        var offender = await r.ReadULong(cancellationToken).ConfigureAwait(false);

        if (complaintType.Value is Tbc.SpamType.Mail) {
        ...
        else if (complaintType.Value is Tbc.SpamType.Chat) {
