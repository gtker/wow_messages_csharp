...
public class SMSG_PETITION_SHOWLIST: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_PETITION_SHOWLIST> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfPetitions; ++i) {
            petitions.Add(await Tbc.PetitionShowlist.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
