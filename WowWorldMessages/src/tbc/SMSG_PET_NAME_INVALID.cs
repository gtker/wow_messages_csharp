...
public class SMSG_PET_NAME_INVALID: TbcServerMessage, IWorldMessage {
    ...
    public required Tbc.PetNameInvalidReason Reason { get; set; }
    ...
    internal DeclinedPetNameIncluded IncludedValue => Included.Match(
        _ => Tbc.DeclinedPetNameIncluded.Included,
        v => v
    );
    ...
    public static async Task<SMSG_PET_NAME_INVALID> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var reason = (Tbc.PetNameInvalidReason)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var name = await r.ReadCString(cancellationToken).ConfigureAwait(false);

        DeclinedPetNameIncludedType included = (Tbc.DeclinedPetNameIncluded)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (included.Value is Tbc.DeclinedPetNameIncluded.Included) {
