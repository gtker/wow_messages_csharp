...
public class SMSG_TRADE_STATUS_EXTENDED: TbcServerMessage, IWorldMessage {
    /// <summary>
    ...
    public static async Task<SMSG_TRADE_STATUS_EXTENDED> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < TradeSlotsLength; ++i) {
            tradeSlots[i] = await Tbc.TradeSlot.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
        }
