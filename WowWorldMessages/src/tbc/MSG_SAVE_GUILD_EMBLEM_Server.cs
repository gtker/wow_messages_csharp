...
public class MSG_SAVE_GUILD_EMBLEM_Server: TbcServerMessage, IWorldMessage {
    public required Tbc.GuildEmblemResult Result { get; set; }
    ...
    public static async Task<MSG_SAVE_GUILD_EMBLEM_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var result = (Tbc.GuildEmblemResult)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
