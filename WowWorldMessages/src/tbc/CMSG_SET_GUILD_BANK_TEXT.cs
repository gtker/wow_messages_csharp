...
public class CMSG_SET_GUILD_BANK_TEXT: TbcClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 1034, cancellationToken).ConfigureAwait(false);
