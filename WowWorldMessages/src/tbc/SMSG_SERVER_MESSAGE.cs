...
public class SMSG_SERVER_MESSAGE: TbcServerMessage, IWorldMessage {
    public required Tbc.ServerMessageType MessageType { get; set; }
    ...
    public static async Task<SMSG_SERVER_MESSAGE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var messageType = (Tbc.ServerMessageType)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
