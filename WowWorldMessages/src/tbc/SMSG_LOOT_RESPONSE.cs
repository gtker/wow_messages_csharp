...
public class SMSG_LOOT_RESPONSE: TbcServerMessage, IWorldMessage {
    public class LootMethodError {
        public required Tbc.LootMethodError LootError { get; set; }
    }
    public required ulong Guid { get; set; }
    public required LootMethodType LootMethod { get; set; }
    internal LootMethod LootMethodValue => LootMethod.Match(
        _ => Tbc.LootMethod.Error,
        v => v
    );
    ...
    public static async Task<SMSG_LOOT_RESPONSE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        LootMethodType lootMethod = (Tbc.LootMethod)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (lootMethod.Value is Tbc.LootMethod.Error) {
            var lootError = (Tbc.LootMethodError)await r.ReadByte(cancellationToken).ConfigureAwait(false);
        ...
        for (var i = 0; i < amountOfItems; ++i) {
            items.Add(await Tbc.LootItem.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
