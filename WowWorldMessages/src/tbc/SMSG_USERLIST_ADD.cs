...
public class SMSG_USERLIST_ADD: TbcServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 1007, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_USERLIST_ADD> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
