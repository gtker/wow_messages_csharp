...
public class GuildMember {
    ...
    internal GuildMemberStatus StatusValue => Status.Match(
        _ => Tbc.GuildMemberStatus.Offline,
        v => v
    );
    ...
    public static async Task<GuildMember> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        GuildMemberStatusType status = (Tbc.GuildMemberStatus)await r.ReadByte(cancellationToken).ConfigureAwait(false);
        ...
        if (status.Value is Tbc.GuildMemberStatus.Offline) {
