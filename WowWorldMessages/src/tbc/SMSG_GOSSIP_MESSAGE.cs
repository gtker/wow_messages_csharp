...
public class SMSG_GOSSIP_MESSAGE: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_GOSSIP_MESSAGE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfGossipItems; ++i) {
            gossips.Add(await Tbc.GossipItem.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
        ...
        for (var i = 0; i < amountOfQuests; ++i) {
            quests.Add(await Tbc.QuestItem.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
