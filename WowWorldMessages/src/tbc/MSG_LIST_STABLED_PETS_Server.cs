...
public class MSG_LIST_STABLED_PETS_Server: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<MSG_LIST_STABLED_PETS_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfPets; ++i) {
            pets.Add(await Tbc.StabledPet.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
