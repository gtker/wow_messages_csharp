...
public class SMSG_TITLE_EARNED: TbcServerMessage, IWorldMessage {
    public required uint Title { get; set; }
    public required Tbc.TitleEarnStatus Status { get; set; }
    ...
    public static async Task<SMSG_TITLE_EARNED> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var status = (Tbc.TitleEarnStatus)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
