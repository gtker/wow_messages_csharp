...
public class SMSG_CHARACTER_LOGIN_FAILED: TbcServerMessage, IWorldMessage {
    public required Tbc.WorldResult Result { get; set; }
    ...
    public static async Task<SMSG_CHARACTER_LOGIN_FAILED> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var result = (Tbc.WorldResult)await r.ReadByte(cancellationToken).ConfigureAwait(false);
