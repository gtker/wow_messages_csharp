...
public class SMSG_ITEM_PUSH_RESULT: TbcServerMessage, IWorldMessage {
    public required ulong Guid { get; set; }
    public required Tbc.NewItemSource Source { get; set; }
    public required Tbc.NewItemCreationType CreationType { get; set; }
    public required Tbc.NewItemChatAlert AlertChat { get; set; }
    ...
    public static async Task<SMSG_ITEM_PUSH_RESULT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var source = (Tbc.NewItemSource)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var creationType = (Tbc.NewItemCreationType)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var alertChat = (Tbc.NewItemChatAlert)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
