...
public class SMSG_GMTICKET_GETTICKET: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_GMTICKET_GETTICKET> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        GmTicketStatusType status = (Tbc.GmTicketStatus)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        if (status.Value is Tbc.GmTicketStatus.HasText) {
            var text = await r.ReadCString(cancellationToken).ConfigureAwait(false);

            var ticketType = (Tbc.GmTicketType)await r.ReadByte(cancellationToken).ConfigureAwait(false);
            ...
            var escalationStatus = (Tbc.GmTicketEscalationStatus)await r.ReadByte(cancellationToken).ConfigureAwait(false);
