...
public class SMSG_RAID_INSTANCE_MESSAGE: TbcServerMessage, IWorldMessage {
    public required Tbc.RaidInstanceMessage MessageType { get; set; }
    ...
    public static async Task<SMSG_RAID_INSTANCE_MESSAGE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var messageType = (Tbc.RaidInstanceMessage)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
