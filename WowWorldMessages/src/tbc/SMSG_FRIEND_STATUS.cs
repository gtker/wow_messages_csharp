...
public class SMSG_FRIEND_STATUS: TbcServerMessage, IWorldMessage {
    public required Tbc.FriendResult Result { get; set; }
    ...
    public static async Task<SMSG_FRIEND_STATUS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var result = (Tbc.FriendResult)await r.ReadByte(cancellationToken).ConfigureAwait(false);
