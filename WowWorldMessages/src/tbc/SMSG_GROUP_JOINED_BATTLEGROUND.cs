...
public class SMSG_GROUP_JOINED_BATTLEGROUND: TbcServerMessage, IWorldMessage {
    public required Tbc.BgTypeId Id { get; set; }
    ...
    public static async Task<SMSG_GROUP_JOINED_BATTLEGROUND> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var id = (Tbc.BgTypeId)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
