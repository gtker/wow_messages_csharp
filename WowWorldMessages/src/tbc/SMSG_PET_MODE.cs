...
public class SMSG_PET_MODE: TbcServerMessage, IWorldMessage {
    public required ulong Guid { get; set; }
    public required Tbc.PetReactState ReactState { get; set; }
    public required Tbc.PetCommandState CommandState { get; set; }
    /// <summary>
    /// vmangos sets to 0.
    /// </summary>
    ...
    public static async Task<SMSG_PET_MODE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var reactState = (Tbc.PetReactState)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        var commandState = (Tbc.PetCommandState)await r.ReadByte(cancellationToken).ConfigureAwait(false);
