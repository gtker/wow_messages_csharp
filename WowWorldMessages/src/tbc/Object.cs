...
public class Object {
    public class UpdateTypeCreateObject {
        ...
        public required Tbc.ObjectType ObjectType { get; set; }
    }
    ...
    internal UpdateType UpdateTypeValue => UpdateType.Match(
        _ => Tbc.UpdateType.CreateObject,
        _ => Tbc.UpdateType.CreateObject2,
        _ => Tbc.UpdateType.Movement,
        _ => Tbc.UpdateType.NearObjects,
        _ => Tbc.UpdateType.OutOfRangeObjects,
        _ => Tbc.UpdateType.Values,
        v => v
    );
    ...
    public static async Task<Object> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        UpdateTypeType updateType = (Tbc.UpdateType)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (updateType.Value is Tbc.UpdateType.Values) {
        ...
        else if (updateType.Value is Tbc.UpdateType.Movement) {
        ...
        else if (updateType.Value is Tbc.UpdateType.CreateObject) {
            var guid3 = await r.ReadPackedGuid(cancellationToken).ConfigureAwait(false);

            var objectType = (Tbc.ObjectType)await r.ReadByte(cancellationToken).ConfigureAwait(false);
        ...
        else if (updateType.Value is Tbc.UpdateType.CreateObject2) {
            var guid3 = await r.ReadPackedGuid(cancellationToken).ConfigureAwait(false);
        ...
        else if (updateType.Value is Tbc.UpdateType.OutOfRangeObjects) {
        ...
        else if (updateType.Value is Tbc.UpdateType.NearObjects) {
            // ReSharper disable once UnusedVariable.Compiler
            var count = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
