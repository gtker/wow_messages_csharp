...
public class SMSG_QUESTGIVER_REQUEST_ITEMS: TbcServerMessage, IWorldMessage {
    ...
    public required Tbc.QuestCompletable Completable { get; set; }
    /// <summary>
    ...
    public static async Task<SMSG_QUESTGIVER_REQUEST_ITEMS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfRequiredItems; ++i) {
            requiredItems.Add(await Tbc.QuestItemRequirement.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }

        var completable = (Tbc.QuestCompletable)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
