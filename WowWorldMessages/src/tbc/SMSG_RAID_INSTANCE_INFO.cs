...
public class SMSG_RAID_INSTANCE_INFO: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_RAID_INSTANCE_INFO> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfRaidInfos; ++i) {
            raidInfos.Add(await Tbc.RaidInfo.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
