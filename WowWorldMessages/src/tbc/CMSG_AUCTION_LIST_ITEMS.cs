...
public class CMSG_AUCTION_LIST_ITEMS: TbcClientMessage, IWorldMessage {
    ...
    public required Tbc.ItemQuality AuctionQuality { get; set; }
    ...
    public static async Task<CMSG_AUCTION_LIST_ITEMS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var auctionQuality = (Tbc.ItemQuality)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        for (var i = 0; i < amountOfSortedAuctions; ++i) {
            sortedAuctions.Add(await Tbc.AuctionSort.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
