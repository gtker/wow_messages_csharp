...
public class CMSG_MESSAGECHAT: TbcClientMessage, IWorldMessage {
    ...
    internal ChatType ChatTypeValue => ChatType.Match(
        _ => Tbc.ChatType.Channel,
        _ => Tbc.ChatType.Whisper,
        v => v
    );
    ...
    public static async Task<CMSG_MESSAGECHAT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ChatTypeType chatType = (Tbc.ChatType)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var language = (Tbc.Language)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        if (chatType.Value is Tbc.ChatType.Whisper) {
        ...
        else if (chatType.Value is Tbc.ChatType.Channel) {
