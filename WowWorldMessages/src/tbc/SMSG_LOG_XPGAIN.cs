...
public class SMSG_LOG_XPGAIN: TbcServerMessage, IWorldMessage {
    ...
    internal ExperienceAwardType ExpTypeValue => ExpType.Match(
        _ => Tbc.ExperienceAwardType.NonKill,
        v => v
    );
    ...
    public static async Task<SMSG_LOG_XPGAIN> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        ExperienceAwardTypeType expType = (Tbc.ExperienceAwardType)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (expType.Value is Tbc.ExperienceAwardType.NonKill) {
