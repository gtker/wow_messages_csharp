...
public class SMSG_SEND_MAIL_RESULT: TbcServerMessage, IWorldMessage {
    public class MailActionDeleted {
        ...
        internal MailResultTwo Result2Value => Result2.Match(
            _ => Tbc.MailResultTwo.ErrEquipError,
            v => v
        );
    }
    public class MailActionItemTaken {
        ...
        internal MailResult ResultValue => Result.Match(
            _ => Tbc.MailResult.ErrCannotSendToSelf,
            _ => Tbc.MailResult.ErrCantSendWrappedCod,
            _ => Tbc.MailResult.ErrDisabledForTrialAcc,
            _ => Tbc.MailResult.ErrEquipError,
            _ => Tbc.MailResult.ErrInternalError,
            _ => Tbc.MailResult.ErrMailAndChatSuspended,
            _ => Tbc.MailResult.ErrMailAttachmentInvalid,
            _ => Tbc.MailResult.ErrNotEnoughMoney,
            _ => Tbc.MailResult.ErrNotYourTeam,
            _ => Tbc.MailResult.ErrRecipientCapReached,
            _ => Tbc.MailResult.ErrRecipientNotFound,
            _ => Tbc.MailResult.ErrTooManyAttachments,
            _ => Tbc.MailResult.Ok,
            v => v
        );
    }
    ...
    internal MailAction ActionValue => Action.Match(
        _ => Tbc.MailAction.Deleted,
        _ => Tbc.MailAction.ItemTaken,
        _ => Tbc.MailAction.MadePermanent,
        _ => Tbc.MailAction.MoneyTaken,
        _ => Tbc.MailAction.ReturnedToSender,
        _ => Tbc.MailAction.Send,
        v => v
    );
    ...
    public static async Task<SMSG_SEND_MAIL_RESULT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        MailActionType action = (Tbc.MailAction)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        if (action.Value is Tbc.MailAction.ItemTaken) {
            MailResultType result = (Tbc.MailResult)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

            if (result.Value is Tbc.MailResult.ErrEquipError) {
            ...
            else if (result.Value is Tbc.MailResult.Ok) {
            ...
            else if (result.Value is Tbc.MailResult.ErrCannotSendToSelf) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrNotEnoughMoney) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrRecipientNotFound) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrNotYourTeam) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrInternalError) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrDisabledForTrialAcc) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrRecipientCapReached) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrCantSendWrappedCod) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrMailAndChatSuspended) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrTooManyAttachments) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            ...
            else if (result.Value is Tbc.MailResult.ErrMailAttachmentInvalid) {
                var item = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        else if (action.Value is Tbc.MailAction.Send) {
            MailResultTwoType result2 = (Tbc.MailResultTwo)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

            if (result2.Value is Tbc.MailResultTwo.ErrEquipError) {
        ...
        else if (action.Value is Tbc.MailAction.MoneyTaken) {
            MailResultTwoType result2 = (Tbc.MailResultTwo)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        else if (action.Value is Tbc.MailAction.ReturnedToSender) {
            MailResultTwoType result2 = (Tbc.MailResultTwo)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        else if (action.Value is Tbc.MailAction.Deleted) {
            MailResultTwoType result2 = (Tbc.MailResultTwo)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        else if (action.Value is Tbc.MailAction.MadePermanent) {
            MailResultTwoType result2 = (Tbc.MailResultTwo)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
