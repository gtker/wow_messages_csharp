...
public class SMSG_PET_SPELLS: TbcServerMessage, IWorldMessage {
    public required ulong Pet { get; set; }
    public struct OptionalActionBars {
        public required uint Duration { get; set; }
        public required Tbc.PetReactState React { get; set; }
        public required Tbc.PetCommandState Command { get; set; }
        /// <summary>
        /// mangoszero: set to 0
        /// </summary>
        public required byte Unknown { get; set; }
        public required Tbc.PetEnabled PetEnabled { get; set; }
    ...
    public static async Task<SMSG_PET_SPELLS> ReadBodyAsync(Stream r, uint bodySize, CancellationToken cancellationToken = default) {
        ...
        if (__size < bodySize) {
            ...
            var react = (Tbc.PetReactState)await r.ReadByte(cancellationToken).ConfigureAwait(false);
            __size += 1;

            var command = (Tbc.PetCommandState)await r.ReadByte(cancellationToken).ConfigureAwait(false);
            __size += 1;

            var unknown = await r.ReadByte(cancellationToken).ConfigureAwait(false);
            __size += 1;

            var petEnabled = (Tbc.PetEnabled)await r.ReadByte(cancellationToken).ConfigureAwait(false);
            __size += 1;
            ...
            for (var i = 0; i < amountOfCooldowns; ++i) {
                cooldowns.Add(await Tbc.PetSpellCooldown.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
                __size += 12;
            }
