...
public class SMSG_GUILD_COMMAND_RESULT: TbcServerMessage, IWorldMessage {
    public required Tbc.GuildCommand Command { get; set; }
    public required string StringValue { get; set; }
    public required Tbc.GuildCommandResult Result { get; set; }
    ...
    public static async Task<SMSG_GUILD_COMMAND_RESULT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var command = (Tbc.GuildCommand)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var stringValue = await r.ReadCString(cancellationToken).ConfigureAwait(false);

        var result = (Tbc.GuildCommandResult)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
