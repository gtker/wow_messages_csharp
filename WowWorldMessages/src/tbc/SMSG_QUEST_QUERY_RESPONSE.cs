...
public class SMSG_QUEST_QUERY_RESPONSE: TbcServerMessage, IWorldMessage {
    ...
    /// </summary>
    public required Tbc.Faction ReputationObjectiveFaction { get; set; }
    /// <summary>
    /// cmangos: shown in quest log as part of quest objective
    /// </summary>
    ...
    /// </summary>
    public required Tbc.Faction RequiredOppositeFaction { get; set; }
    /// <summary>
    ...
    public static async Task<SMSG_QUEST_QUERY_RESPONSE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var reputationObjectiveFaction = (Tbc.Faction)await r.ReadUShort(cancellationToken).ConfigureAwait(false);

        var reputationObjectiveValue = await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var requiredOppositeFaction = (Tbc.Faction)await r.ReadUShort(cancellationToken).ConfigureAwait(false);
        ...
        for (var i = 0; i < RewardsLength; ++i) {
            rewards[i] = await Tbc.QuestItemReward.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
        }

        var choiceRewards = new QuestItemReward[ChoiceRewardsLength];
        for (var i = 0; i < ChoiceRewardsLength; ++i) {
            choiceRewards[i] = await Tbc.QuestItemReward.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
        }
        ...
        for (var i = 0; i < ObjectivesLength; ++i) {
            objectives[i] = await Tbc.QuestObjective.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
        }
