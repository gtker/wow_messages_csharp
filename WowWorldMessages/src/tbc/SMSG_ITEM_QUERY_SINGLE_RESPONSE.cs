...
public class SMSG_ITEM_QUERY_SINGLE_RESPONSE: TbcServerMessage, IWorldMessage {
    public required uint Item { get; set; }
    public struct OptionalFound {
        public required Tbc.ItemClassAndSubClass ClassAndSubClass { get; set; }
        /// <summary>
        ...
        public required Tbc.ItemQuality Quality { get; set; }
        public required Tbc.ItemFlag Flags { get; set; }
        ...
        public required Tbc.AllowedClass AllowedClass { get; set; }
        public required Tbc.AllowedRace AllowedRace { get; set; }
        ...
        public required Tbc.Faction RequiredFaction { get; set; }
        /// <summary>
        ...
        public required Tbc.Bonding Bonding { get; set; }
        public required string Description { get; set; }
        public required uint PageText { get; set; }
        public required Tbc.Language Language { get; set; }
        public required Tbc.PageTextMaterial PageTextMaterial { get; set; }
        /// <summary>
        ...
        public required Tbc.SheatheType SheatheType { get; set; }
        /// <summary>
    ...
    public static async Task<SMSG_ITEM_QUERY_SINGLE_RESPONSE> ReadBodyAsync(Stream r, uint bodySize, CancellationToken cancellationToken = default) {
        ...
        if (size < bodySize) {
            var classAndSubClass = (Tbc.ItemClassAndSubClass)await r.ReadULong(cancellationToken).ConfigureAwait(false);
            size += 8;
            ...
            var quality = (Tbc.ItemQuality)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            size += 4;
            ...
            var inventoryType = (Tbc.InventoryType)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            size += 4;
            ...
            var requiredFaction = (Tbc.Faction)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            size += 4;
            ...
            for (var i = 0; i < OptionalFound.StatsLength; ++i) {
                stats[i] = await Tbc.ItemStat.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
                size += 8;
            }

            var damages = new ItemDamageType[OptionalFound.DamagesLength];
            for (var i = 0; i < OptionalFound.DamagesLength; ++i) {
                damages[i] = await Tbc.ItemDamageType.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
                size += 12;
            }
            ...
            for (var i = 0; i < OptionalFound.SpellsLength; ++i) {
                spells[i] = await Tbc.ItemSpells.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
                size += 24;
            }

            var bonding = (Tbc.Bonding)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            size += 4;
            ...
            var language = (Tbc.Language)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            size += 4;

            var pageTextMaterial = (Tbc.PageTextMaterial)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            size += 4;
            ...
            var sheatheType = (Tbc.SheatheType)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            size += 4;
            ...
            var itemSet = (Tbc.ItemSet)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            size += 4;
            ...
            for (var i = 0; i < OptionalFound.SocketsLength; ++i) {
                sockets[i] = await Tbc.ItemSocket.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
                size += 8;
            }
