...
public class SMSG_CHANNEL_LIST: TbcServerMessage, IWorldMessage {
    public required string ChannelName { get; set; }
    public required Tbc.ChannelFlags ChannelFlags { get; set; }
    ...
    public static async Task<SMSG_CHANNEL_LIST> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfMembers; ++i) {
            members.Add(await Tbc.ChannelMember.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
