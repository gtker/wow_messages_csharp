...
public class SMSG_PET_NAME_QUERY_RESPONSE: TbcServerMessage, IWorldMessage {
    ...
    internal PetQueryDisabledNames NamesValue => Names.Match(
        _ => Tbc.PetQueryDisabledNames.Present,
        v => v
    );
    ...
    public static async Task<SMSG_PET_NAME_QUERY_RESPONSE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        PetQueryDisabledNamesType names = (Tbc.PetQueryDisabledNames)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (names.Value is Tbc.PetQueryDisabledNames.Present) {
