...
public class SMSG_RAID_GROUP_ONLY: TbcServerMessage, IWorldMessage {
    public required uint HomebindTimer { get; set; }
    public required Tbc.RaidGroupError Error { get; set; }
    ...
    public static async Task<SMSG_RAID_GROUP_ONLY> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var error = (Tbc.RaidGroupError)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
