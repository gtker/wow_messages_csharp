...
public class MSG_CORPSE_QUERY_Server: TbcServerMessage, IWorldMessage {
    public class CorpseQueryResultFound {
        public required Tbc.Map CorpseMap { get; set; }
    ...
    public static async Task<MSG_CORPSE_QUERY_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        CorpseQueryResultType result = (Tbc.CorpseQueryResult)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (result.Value is Tbc.CorpseQueryResult.Found) {
            ...
            var corpseMap = (Tbc.Map)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
