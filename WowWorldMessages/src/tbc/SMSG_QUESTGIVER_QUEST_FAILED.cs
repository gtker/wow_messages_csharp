...
public class SMSG_QUESTGIVER_QUEST_FAILED: TbcServerMessage, IWorldMessage {
    public required uint QuestId { get; set; }
    public required Tbc.QuestFailedReason Reason { get; set; }
    ...
    public static async Task<SMSG_QUESTGIVER_QUEST_FAILED> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var reason = (Tbc.QuestFailedReason)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
