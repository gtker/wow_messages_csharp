...
public class Mail {
    ...
    internal MailType MessageTypeValue => MessageType.Match(
        _ => Tbc.MailType.Auction,
        _ => Tbc.MailType.Creature,
        _ => Tbc.MailType.Gameobject,
        _ => Tbc.MailType.Item,
        _ => Tbc.MailType.Normal,
        v => v
    );
    ...
    public static async Task<Mail> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        MailTypeType messageType = (Tbc.MailType)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (messageType.Value is Tbc.MailType.Normal) {
        ...
        else if (messageType.Value is Tbc.MailType.Creature) {
        ...
        else if (messageType.Value is Tbc.MailType.Gameobject) {
            var senderId = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        else if (messageType.Value is Tbc.MailType.Auction) {
        ...
        else if (messageType.Value is Tbc.MailType.Item) {
        ...
        for (var i = 0; i < amountOfItems; ++i) {
            items.Add(await Tbc.MailListItem.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
