...
public class SMSG_TRIGGER_CINEMATIC: TbcServerMessage, IWorldMessage {
    public required Tbc.CinematicSequenceId CinematicSequenceId { get; set; }
    ...
    public static async Task<SMSG_TRIGGER_CINEMATIC> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var cinematicSequenceId = (Tbc.CinematicSequenceId)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
