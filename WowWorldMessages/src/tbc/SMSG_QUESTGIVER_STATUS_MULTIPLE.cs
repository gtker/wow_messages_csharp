...
public class SMSG_QUESTGIVER_STATUS_MULTIPLE: TbcServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 1047, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_QUESTGIVER_STATUS_MULTIPLE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfStatuses; ++i) {
            statuses.Add(await Tbc.QuestGiverStatusReport.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
