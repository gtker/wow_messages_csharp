...
public class SMSG_NPC_TEXT_UPDATE: TbcServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_NPC_TEXT_UPDATE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < TextsLength; ++i) {
            texts[i] = await Tbc.NpcTextUpdate.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
        }
