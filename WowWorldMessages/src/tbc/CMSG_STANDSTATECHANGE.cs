...
public class CMSG_STANDSTATECHANGE: TbcClientMessage, IWorldMessage {
    public required Tbc.UnitStandState AnimationState { get; set; }
    ...
    public static async Task<CMSG_STANDSTATECHANGE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var animationState = (Tbc.UnitStandState)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
