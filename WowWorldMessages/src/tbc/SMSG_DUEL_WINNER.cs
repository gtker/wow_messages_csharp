...
public class SMSG_DUEL_WINNER: TbcServerMessage, IWorldMessage {
    public required Tbc.DuelWinnerReason Reason { get; set; }
    ...
    public static async Task<SMSG_DUEL_WINNER> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var reason = (Tbc.DuelWinnerReason)await r.ReadByte(cancellationToken).ConfigureAwait(false);
