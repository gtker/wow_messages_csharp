...
public class MSG_MOVE_START_PITCH_UP_Server: VanillaServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 191, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_MOVE_START_PITCH_UP_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new MSG_MOVE_START_PITCH_UP_Server {
            Guid = guid,
            Info = info,
        };
    }
