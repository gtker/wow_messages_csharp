...
public class SMSG_FORCE_RUN_BACK_SPEED_CHANGE: VanillaServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 228, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_FORCE_RUN_BACK_SPEED_CHANGE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new SMSG_FORCE_RUN_BACK_SPEED_CHANGE {
            Guid = guid,
            MoveEvent = moveEvent,
            Speed = speed,
        };
