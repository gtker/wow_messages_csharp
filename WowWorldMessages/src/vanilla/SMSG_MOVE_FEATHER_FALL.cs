...
public class SMSG_MOVE_FEATHER_FALL: VanillaServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 242, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_MOVE_FEATHER_FALL> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new SMSG_MOVE_FEATHER_FALL {
            Guid = guid,
            Counter = counter,
        };
    }
