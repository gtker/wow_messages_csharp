...
public class CMSG_CHANNEL_BAN: VanillaClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 165, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_CHANNEL_BAN> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
