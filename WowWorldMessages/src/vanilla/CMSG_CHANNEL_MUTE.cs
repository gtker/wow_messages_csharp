...
public class CMSG_CHANNEL_MUTE: VanillaClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 161, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_CHANNEL_MUTE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
