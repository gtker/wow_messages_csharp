...
public class SMSG_MOVE_WATER_WALK: VanillaServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 222, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_MOVE_WATER_WALK> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new SMSG_MOVE_WATER_WALK {
            Guid = guid,
            Counter = counter,
        };
    }
