...
public class CMSG_FORCE_WALK_SPEED_CHANGE_ACK: VanillaClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 731, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_FORCE_WALK_SPEED_CHANGE_ACK> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new CMSG_FORCE_WALK_SPEED_CHANGE_ACK {
            Guid = guid,
            Counter = counter,
            Info = info,
            NewSpeed = newSpeed,
