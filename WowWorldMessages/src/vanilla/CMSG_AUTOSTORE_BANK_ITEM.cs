...
public class CMSG_AUTOSTORE_BANK_ITEM: VanillaClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, 6, 642, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_AUTOSTORE_BANK_ITEM> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new CMSG_AUTOSTORE_BANK_ITEM {
            BagIndex = bagIndex,
            SlotIndex = slotIndex,
        };
    }
