...
public class CMSG_CHANNEL_UNMODERATOR: VanillaClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 160, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_CHANNEL_UNMODERATOR> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new CMSG_CHANNEL_UNMODERATOR {
