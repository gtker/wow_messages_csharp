...
public class SMSG_RAID_GROUP_ONLY: WrathServerMessage, IWorldMessage {
    public required uint HomebindTimer { get; set; }
    public required Wrath.RaidGroupError Error { get; set; }
    ...
    public static async Task<SMSG_RAID_GROUP_ONLY> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var error = (Wrath.RaidGroupError)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
