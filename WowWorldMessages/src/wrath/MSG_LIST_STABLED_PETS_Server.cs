...
public class MSG_LIST_STABLED_PETS_Server: WrathServerMessage, IWorldMessage {
    ...
    public static async Task<MSG_LIST_STABLED_PETS_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfPets; ++i) {
            pets.Add(await Wrath.StabledPet.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
