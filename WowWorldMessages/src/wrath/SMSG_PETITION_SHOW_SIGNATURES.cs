...
public class SMSG_PETITION_SHOW_SIGNATURES: WrathServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_PETITION_SHOW_SIGNATURES> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfSignatures; ++i) {
            signatures.Add(await Wrath.PetitionSignature.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
