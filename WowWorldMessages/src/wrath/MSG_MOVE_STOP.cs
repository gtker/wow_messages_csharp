...
public class MSG_MOVE_STOP: WrathClientMessage, WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 183, cancellationToken).ConfigureAwait(false);
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 183, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_MOVE_STOP> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
