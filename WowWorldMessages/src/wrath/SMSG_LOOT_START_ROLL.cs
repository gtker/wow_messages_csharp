...
public class SMSG_LOOT_START_ROLL: WrathServerMessage, IWorldMessage {
    ...
    public required Wrath.RollFlags Flags { get; set; }
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, 35, 673, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_LOOT_START_ROLL> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var flags = (RollFlags)await r.ReadByte(cancellationToken).ConfigureAwait(false);
