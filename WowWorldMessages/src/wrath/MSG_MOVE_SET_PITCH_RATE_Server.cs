...
public class MSG_MOVE_SET_PITCH_RATE_Server: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 1115, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_MOVE_SET_PITCH_RATE_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new MSG_MOVE_SET_PITCH_RATE_Server {
            Player = player,
            Info = info,
            NewSpeed = newSpeed,
        };
