...
public class SMSG_SPELLSTEALLOG: WrathServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_SPELLSTEALLOG> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfSpellSteals; ++i) {
            spellSteals.Add(await Wrath.SpellSteal.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
