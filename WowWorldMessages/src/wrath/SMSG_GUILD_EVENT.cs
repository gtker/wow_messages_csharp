...
public class SMSG_GUILD_EVENT: WrathServerMessage, IWorldMessage {
    public required Wrath.GuildEvent EventValue { get; set; }
    ...
    public static async Task<SMSG_GUILD_EVENT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var eventValue = (Wrath.GuildEvent)await r.ReadByte(cancellationToken).ConfigureAwait(false);
