...
public class MSG_PARTY_ASSIGNMENT_Client: WrathClientMessage, IWorldMessage {
    public required Wrath.PartyRole Role { get; set; }
    ...
    public static async Task<MSG_PARTY_ASSIGNMENT_Client> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var role = (Wrath.PartyRole)await r.ReadByte(cancellationToken).ConfigureAwait(false);
