...
public class SMSG_RAID_INSTANCE_INFO: WrathServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_RAID_INSTANCE_INFO> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfRaidInfos; ++i) {
            raidInfos.Add(await Wrath.RaidInfo.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
