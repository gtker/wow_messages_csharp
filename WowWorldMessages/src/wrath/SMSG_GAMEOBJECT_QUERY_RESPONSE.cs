...
public class SMSG_GAMEOBJECT_QUERY_RESPONSE: WrathServerMessage, IWorldMessage {
    /// <summary>
    ...
    public struct OptionalFound {
        ...
        public const int GameobjectQuestItemsLength = 6;
        public required uint[] GameobjectQuestItems { get; set; }
    }
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        if (Found is { } found) {
            ...
            foreach (var v in found.GameobjectQuestItems) {
                await w.WriteUInt(v, cancellationToken).ConfigureAwait(false);
            }
    ...
    public static async Task<SMSG_GAMEOBJECT_QUERY_RESPONSE> ReadBodyAsync(Stream r, uint bodySize, CancellationToken cancellationToken = default) {
        ...
        if (__size < bodySize) {
            ...
            var gameobjectQuestItems = new uint[OptionalFound.GameobjectQuestItemsLength];
            for (var i = 0; i < OptionalFound.GameobjectQuestItemsLength; ++i) {
                gameobjectQuestItems[i] = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
                __size += 4;
            }
            ...
            optionalFound = new OptionalFound {
                ...
                GameobjectQuestItems = gameobjectQuestItems,
            };
        }
    ...
    internal int Size() {
        ...
        if (Found is { } found) {
            ...
            // gameobject_quest_items: Generator.Generated.DataTypeArray
            size += found.GameobjectQuestItems.Sum(e => 4);
