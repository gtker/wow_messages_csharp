...
public class SMSG_TRADE_STATUS_EXTENDED: WrathServerMessage, IWorldMessage {
    /// <summary>
    ...
    /// </summary>
    public required uint TradeId { get; set; }
    /// <summary>
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        await w.WriteUInt(TradeId, cancellationToken).ConfigureAwait(false);
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, 534, 289, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_TRADE_STATUS_EXTENDED> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var tradeId = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        for (var i = 0; i < TradeSlotsLength; ++i) {
            tradeSlots[i] = await Wrath.TradeSlot.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
        }
