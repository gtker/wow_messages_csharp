...
public class SMSG_TRADE_STATUS: WrathServerMessage, IWorldMessage {
    ...
    internal TradeStatus StatusValue => Status.Match(
        _ => Wrath.TradeStatus.BeginTrade,
        _ => Wrath.TradeStatus.CloseWindow,
        _ => Wrath.TradeStatus.NotOnTaplist,
        _ => Wrath.TradeStatus.OnlyConjured,
        v => v
    );
    ...
    public static async Task<SMSG_TRADE_STATUS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        TradeStatusType status = (Wrath.TradeStatus)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        if (status.Value is Wrath.TradeStatus.BeginTrade) {
        ...
        else if (status.Value is Wrath.TradeStatus.CloseWindow) {
            var inventoryResult = (Wrath.InventoryResult)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        else if (status.Value is Wrath.TradeStatus.OnlyConjured) {
        ...
        else if (status.Value is Wrath.TradeStatus.NotOnTaplist) {
            var slot = await r.ReadByte(cancellationToken).ConfigureAwait(false);
