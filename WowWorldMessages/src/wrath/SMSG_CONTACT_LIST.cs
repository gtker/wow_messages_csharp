...
public class SMSG_CONTACT_LIST: WrathServerMessage, IWorldMessage {
    /// <summary>
    /// Indicates which kinds of relations are being sent in this list
    /// </summary>
    public required Wrath.RelationType ListMask { get; set; }
    ...
    public static async Task<SMSG_CONTACT_LIST> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfRelations; ++i) {
            relations.Add(await Wrath.Relation.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
