...
public class CMSG_SETSHEATHED: WrathClientMessage, IWorldMessage {
    public required Wrath.SheathState Sheathed { get; set; }
    ...
    public static async Task<CMSG_SETSHEATHED> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var sheathed = (Wrath.SheathState)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
