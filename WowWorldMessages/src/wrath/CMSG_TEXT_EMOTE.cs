...
public class CMSG_TEXT_EMOTE: WrathClientMessage, IWorldMessage {
    public required Wrath.TextEmote TextEmote { get; set; }
    ...
    public static async Task<CMSG_TEXT_EMOTE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var textEmote = (Wrath.TextEmote)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
