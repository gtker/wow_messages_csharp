...
using LfgJoinStatusType = OneOf.OneOf<SMSG_LFG_UPDATE_PARTY.LfgJoinStatusJoined, LfgJoinStatus>;

[System.CodeDom.Compiler.GeneratedCode("WoWM", "0.1.0")]
// ReSharper disable once InconsistentNaming
public class SMSG_LFG_UPDATE_PARTY: WrathServerMessage, IWorldMessage {
    public class LfgJoinStatusJoined {
        ...
        public required byte Joined { get; set; }
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        if (JoinStatus.Value is SMSG_LFG_UPDATE_PARTY.LfgJoinStatusJoined lfgJoinStatusJoined) {
            await w.WriteByte(lfgJoinStatusJoined.Joined, cancellationToken).ConfigureAwait(false);
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 872, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_LFG_UPDATE_PARTY> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        if (joinStatus.Value is Wrath.LfgJoinStatus.Joined) {
            var joined = await r.ReadByte(cancellationToken).ConfigureAwait(false);
        ...
        return new SMSG_LFG_UPDATE_PARTY {
    ...
    internal int Size() {
        ...
        if (JoinStatus.Value is SMSG_LFG_UPDATE_PARTY.LfgJoinStatusJoined lfgJoinStatusJoined) {
            // joined: Generator.Generated.DataTypeInteger
            size += 1;
