...
public class SMSG_PARTY_COMMAND_RESULT: WrathServerMessage, IWorldMessage {
    public required Wrath.PartyOperation Operation { get; set; }
    public required string Member { get; set; }
    public required Wrath.PartyResult Result { get; set; }
    ...
    public static async Task<SMSG_PARTY_COMMAND_RESULT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var operation = (Wrath.PartyOperation)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var member = await r.ReadCString(cancellationToken).ConfigureAwait(false);

        var result = (Wrath.PartyResult)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
