...
public class SMSG_GMTICKET_DELETETICKET: WrathServerMessage, IWorldMessage {
    public required Wrath.GmTicketResponse Response { get; set; }
    ...
    public static async Task<SMSG_GMTICKET_DELETETICKET> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var response = (Wrath.GmTicketResponse)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
