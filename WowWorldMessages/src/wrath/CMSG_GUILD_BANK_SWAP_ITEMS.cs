...
public class CMSG_GUILD_BANK_SWAP_ITEMS: WrathClientMessage, IWorldMessage {
    ...
    public class BankSwapSourceInventory {
        ...
        internal BankSwapStoreMode ModeValue => Mode.Match(
            _ => Wrath.BankSwapStoreMode.Automatic,
            _ => Wrath.BankSwapStoreMode.Manual,
            v => v
        );
    }
    ...
    public class BankSwapStoreModeManual {
        ...
        public required uint SplitAmount { get; set; }
    }
    ...
    internal BankSwapSource SourceValue => Source.Match(
        _ => Wrath.BankSwapSource.Bank,
        _ => Wrath.BankSwapSource.Inventory,
        v => v
    );
    /// <summary>
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        if (Source.Value is CMSG_GUILD_BANK_SWAP_ITEMS.BankSwapSourceBank bankSwapSourceBank) {
            ...
            await w.WriteUInt(bankSwapSourceBank.Amount, cancellationToken).ConfigureAwait(false);
            ...
            if (bankSwapSourceInventory.Mode.Value is CMSG_GUILD_BANK_SWAP_ITEMS.BankSwapStoreModeAutomatic bankSwapStoreModeAutomatic) {
                ...
                await w.WriteUInt(bankSwapStoreModeAutomatic.Unknown4, cancellationToken).ConfigureAwait(false);
                ...
                await w.WriteUInt(bankSwapStoreModeManual.SplitAmount, cancellationToken).ConfigureAwait(false);
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 1001, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_GUILD_BANK_SWAP_ITEMS> ReadBodyAsync(Stream r, uint bodySize, CancellationToken cancellationToken = default) {
        ...
        BankSwapSourceType source = (Wrath.BankSwapSource)await r.ReadByte(cancellationToken).ConfigureAwait(false);
        __size += 1;

        if (source.Value is Wrath.BankSwapSource.Bank) {
        ...
        else if (source.Value is Wrath.BankSwapSource.Inventory) {
            ...
            BankSwapStoreModeType mode = (Wrath.BankSwapStoreMode)await r.ReadByte(cancellationToken).ConfigureAwait(false);
            __size += 1;

            if (mode.Value is Wrath.BankSwapStoreMode.Automatic) {
            ...
            else if (mode.Value is Wrath.BankSwapStoreMode.Manual) {
                ...
                var splitAmount = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
                __size += 4;
