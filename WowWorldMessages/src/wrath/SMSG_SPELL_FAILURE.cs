...
public class SMSG_SPELL_FAILURE: WrathServerMessage, IWorldMessage {
    public required ulong Guid { get; set; }
    public required byte ExtraCasts { get; set; }
    public required uint Spell { get; set; }
    public required Wrath.SpellCastResult Result { get; set; }
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, 16, 307, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_SPELL_FAILURE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var result = (Wrath.SpellCastResult)await r.ReadByte(cancellationToken).ConfigureAwait(false);
