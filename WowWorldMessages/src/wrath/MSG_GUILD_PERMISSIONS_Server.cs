...
public class MSG_GUILD_PERMISSIONS_Server: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, 63, 1021, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_GUILD_PERMISSIONS_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < BankTabsLength; ++i) {
            bankTabs[i] = await Wrath.BankTab.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
        }
