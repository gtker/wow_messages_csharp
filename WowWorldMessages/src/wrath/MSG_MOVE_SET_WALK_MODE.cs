...
public class MSG_MOVE_SET_WALK_MODE: WrathClientMessage, WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 195, cancellationToken).ConfigureAwait(false);
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 195, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_MOVE_SET_WALK_MODE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new MSG_MOVE_SET_WALK_MODE {
            Guid = guid,
            Info = info,
        };
    }
