...
public class SMSG_SPLINE_SET_FLIGHT_SPEED: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 901, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_SPLINE_SET_FLIGHT_SPEED> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new SMSG_SPLINE_SET_FLIGHT_SPEED {
            Guid = guid,
            Speed = speed,
        };
    }
