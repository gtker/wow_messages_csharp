...
public class SMSG_SET_FORCED_REACTIONS: WrathServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_SET_FORCED_REACTIONS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfReactions; ++i) {
            reactions.Add(await Wrath.ForcedReaction.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
