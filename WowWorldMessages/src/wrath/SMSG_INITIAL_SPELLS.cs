...
public class SMSG_INITIAL_SPELLS: WrathServerMessage, IWorldMessage {
    /// <summary>
    ...
    public static async Task<SMSG_INITIAL_SPELLS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < spellCount; ++i) {
            initialSpells.Add(await Wrath.InitialSpell.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
        ...
        for (var i = 0; i < cooldownCount; ++i) {
            cooldowns.Add(await Wrath.CooldownSpell.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
