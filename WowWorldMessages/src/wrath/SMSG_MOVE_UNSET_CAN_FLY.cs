...
public class SMSG_MOVE_UNSET_CAN_FLY: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 836, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_MOVE_UNSET_CAN_FLY> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new SMSG_MOVE_UNSET_CAN_FLY {
            Player = player,
            Counter = counter,
        };
    }
