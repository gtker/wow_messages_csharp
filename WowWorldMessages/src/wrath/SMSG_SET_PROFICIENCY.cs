...
public class SMSG_SET_PROFICIENCY: WrathServerMessage, IWorldMessage {
    public required Wrath.ItemClass ClassType { get; set; }
    ...
    public static async Task<SMSG_SET_PROFICIENCY> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var classType = (Wrath.ItemClass)await r.ReadByte(cancellationToken).ConfigureAwait(false);
