...
public class SMSG_THREAT_UPDATE: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 1155, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_THREAT_UPDATE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
