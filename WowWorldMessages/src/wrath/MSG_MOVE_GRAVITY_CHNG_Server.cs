...
public class MSG_MOVE_GRAVITY_CHNG_Server: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 1234, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_MOVE_GRAVITY_CHNG_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new MSG_MOVE_GRAVITY_CHNG_Server {
            Player = player,
            Info = info,
        };
    }
