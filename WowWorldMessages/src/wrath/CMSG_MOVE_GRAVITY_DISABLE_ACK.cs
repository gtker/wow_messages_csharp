...
public class CMSG_MOVE_GRAVITY_DISABLE_ACK: WrathClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 1231, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_MOVE_GRAVITY_DISABLE_ACK> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new CMSG_MOVE_GRAVITY_DISABLE_ACK {
            Guid = guid,
            Unknown = unknown,
            Info = info,
        };
