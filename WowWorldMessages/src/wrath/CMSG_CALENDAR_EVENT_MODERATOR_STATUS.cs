...
public class CMSG_CALENDAR_EVENT_MODERATOR_STATUS: WrathClientMessage, IWorldMessage {
    ...
    public required Wrath.CalendarModeratorRank Rank { get; set; }
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        await w.WriteByte((byte)Rank, cancellationToken).ConfigureAwait(false);

    }

    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, 29, 1077, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_CALENDAR_EVENT_MODERATOR_STATUS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var rank = (Wrath.CalendarModeratorRank)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        return new CMSG_CALENDAR_EVENT_MODERATOR_STATUS {
