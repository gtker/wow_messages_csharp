...
public class SMSG_QUESTGIVER_QUEST_FAILED: WrathServerMessage, IWorldMessage {
    public required uint QuestId { get; set; }
    public required Wrath.QuestFailedReason Reason { get; set; }
    ...
    public static async Task<SMSG_QUESTGIVER_QUEST_FAILED> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var reason = (Wrath.QuestFailedReason)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
