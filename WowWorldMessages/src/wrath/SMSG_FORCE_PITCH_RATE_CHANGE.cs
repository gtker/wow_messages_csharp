...
public class SMSG_FORCE_PITCH_RATE_CHANGE: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 1116, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_FORCE_PITCH_RATE_CHANGE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new SMSG_FORCE_PITCH_RATE_CHANGE {
            Guid = guid,
            MoveEvent = moveEvent,
            Speed = speed,
        };
