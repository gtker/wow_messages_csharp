...
public class MSG_RAID_TARGET_UPDATE_Server: WrathServerMessage, IWorldMessage {
    ...
    internal RaidTargetUpdateType UpdateTypeValue => UpdateType.Match(
        _ => Wrath.RaidTargetUpdateType.Full,
        _ => Wrath.RaidTargetUpdateType.Partial,
        v => v
    );
    ...
    public static async Task<MSG_RAID_TARGET_UPDATE_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        RaidTargetUpdateTypeType updateType = (Wrath.RaidTargetUpdateType)await r.ReadByte(cancellationToken).ConfigureAwait(false);

        if (updateType.Value is Wrath.RaidTargetUpdateType.Full) {
            ...
            for (var i = 0; i < RaidTargetUpdateTypeFull.RaidTargetsLength; ++i) {
                raidTargets[i] = await Wrath.RaidTargetUpdate.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false);
            }
        ...
        else if (updateType.Value is Wrath.RaidTargetUpdateType.Partial) {
