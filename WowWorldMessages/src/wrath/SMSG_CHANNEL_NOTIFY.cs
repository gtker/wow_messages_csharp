...
public class SMSG_CHANNEL_NOTIFY: WrathServerMessage, IWorldMessage {
    public required Wrath.ChatNotify NotifyType { get; set; }
    ...
    public static async Task<SMSG_CHANNEL_NOTIFY> ReadBodyAsync(Stream r, uint bodySize, CancellationToken cancellationToken = default) {
        // ReSharper disable once InconsistentNaming
        var __size = 0;
        var notifyType = (Wrath.ChatNotify)await r.ReadByte(cancellationToken).ConfigureAwait(false);
        __size += 1;
