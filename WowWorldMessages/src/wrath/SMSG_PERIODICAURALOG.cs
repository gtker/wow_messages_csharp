...
public class SMSG_PERIODICAURALOG: WrathServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_PERIODICAURALOG> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfAuras; ++i) {
            auras.Add(await Wrath.AuraLog.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
