...
public class CMSG_LOOT_METHOD: WrathClientMessage, IWorldMessage {
    ...
    public static async Task<CMSG_LOOT_METHOD> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var lootSetting = (Wrath.GroupLootSetting)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var lootMaster = await r.ReadULong(cancellationToken).ConfigureAwait(false);

        var lootThreshold = (Wrath.ItemQuality)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
