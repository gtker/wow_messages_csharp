...
public class CMSG_ARENA_TEAM_LEADER: WrathClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 854, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_ARENA_TEAM_LEADER> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new CMSG_ARENA_TEAM_LEADER {
            ArenaTeam = arenaTeam,
            Player = player,
        };
    }
