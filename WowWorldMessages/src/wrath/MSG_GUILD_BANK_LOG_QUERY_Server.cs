...
public class MSG_GUILD_BANK_LOG_QUERY_Server: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 1006, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_GUILD_BANK_LOG_QUERY_Server> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfMoneyLogs; ++i) {
            moneyLogs.Add(await Wrath.MoneyLogItem.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
