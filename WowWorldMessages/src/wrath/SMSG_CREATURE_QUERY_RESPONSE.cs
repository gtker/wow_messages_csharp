...
public class SMSG_CREATURE_QUERY_RESPONSE: WrathServerMessage, IWorldMessage {
    /// <summary>
    ...
    public struct OptionalFound {
        ...
        public required Wrath.CreatureFamily CreatureFamily { get; set; }
        /// <summary>
        ...
        /// </summary>
        public required uint KillCredit1 { get; set; }
        /// <summary>
        /// mangosone: new in 3.1
        /// </summary>
        public required uint KillCredit2 { get; set; }
        ...
        public const int QuestItemsLength = 6;
        public required uint[] QuestItems { get; set; }
        /// <summary>
        /// mangosone: CreatureMovementInfo.dbc
        /// </summary>
        public required uint MovementId { get; set; }
    }
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        if (Found is { } found) {
            ...
            await w.WriteUInt(found.KillCredit1, cancellationToken).ConfigureAwait(false);

            await w.WriteUInt(found.KillCredit2, cancellationToken).ConfigureAwait(false);
            ...
            foreach (var v in found.QuestItems) {
                await w.WriteUInt(v, cancellationToken).ConfigureAwait(false);
            }

            await w.WriteUInt(found.MovementId, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_CREATURE_QUERY_RESPONSE> ReadBodyAsync(Stream r, uint bodySize, CancellationToken cancellationToken = default) {
        ...
        if (__size < bodySize) {
            ...
            var creatureFamily = (Wrath.CreatureFamily)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            __size += 4;

            var creatureRank = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            __size += 4;

            var killCredit1 = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            __size += 4;

            var killCredit2 = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            __size += 4;
            ...
            var questItems = new uint[OptionalFound.QuestItemsLength];
            for (var i = 0; i < OptionalFound.QuestItemsLength; ++i) {
                questItems[i] = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
                __size += 4;
            }

            var movementId = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            __size += 4;
    ...
    internal int Size() {
        ...
        if (Found is { } found) {
            ...
            // kill_credit1: Generator.Generated.DataTypeInteger
            size += 4;

            // kill_credit2: Generator.Generated.DataTypeInteger
            size += 4;
            ...
            size += found.QuestItems.Sum(e => 4);

            // movement_id: Generator.Generated.DataTypeInteger
            size += 4;
