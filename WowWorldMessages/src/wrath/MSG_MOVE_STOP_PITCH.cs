...
public class MSG_MOVE_STOP_PITCH: WrathClientMessage, WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 193, cancellationToken).ConfigureAwait(false);
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 193, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_MOVE_STOP_PITCH> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new MSG_MOVE_STOP_PITCH {
            Guid = guid,
            Info = info,
        };
    }
