...
public class SMSG_PET_SPELLS: WrathServerMessage, IWorldMessage {
    public required ulong Pet { get; set; }
    public struct OptionalActionBars {
        public required Wrath.CreatureFamily Family { get; set; }
        public required uint Duration { get; set; }
        public required Wrath.PetReactState React { get; set; }
        public required Wrath.PetCommandState Command { get; set; }
        /// <summary>
        /// mangoszero: set to 0
        /// </summary>
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        if (ActionBars is { } actionBars) {
            await w.WriteUShort((ushort)actionBars.Family, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_PET_SPELLS> ReadBodyAsync(Stream r, uint bodySize, CancellationToken cancellationToken = default) {
        ...
        if (__size < bodySize) {
            var family = (Wrath.CreatureFamily)await r.ReadUShort(cancellationToken).ConfigureAwait(false);
            __size += 2;

            var duration = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
            __size += 4;

            var react = (Wrath.PetReactState)await r.ReadByte(cancellationToken).ConfigureAwait(false);
            __size += 1;

            var command = (Wrath.PetCommandState)await r.ReadByte(cancellationToken).ConfigureAwait(false);
            __size += 1;
            ...
            for (var i = 0; i < amountOfCooldowns; ++i) {
                cooldowns.Add(await Wrath.PetSpellCooldown.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
                __size += 14;
            }
    ...
    internal int Size() {
        ...
        if (ActionBars is { } actionBars) {
            // family: Generator.Generated.DataTypeEnum
            size += 2;
            ...
            size += actionBars.Cooldowns.Sum(e => 14);
