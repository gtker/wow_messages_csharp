...
public class SMSG_FORCE_MOVE_UNROOT: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 234, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_FORCE_MOVE_UNROOT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new SMSG_FORCE_MOVE_UNROOT {
            Guid = guid,
            Counter = counter,
        };
    }
