...
public class Relation {
    ...
    public static async Task<Relation> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        if (relationMask.Inner.HasFlag(Wrath.RelationType.Friend)) {
            FriendStatusType status = (Wrath.FriendStatus)await r.ReadByte(cancellationToken).ConfigureAwait(false);

            if (status.Value is Wrath.FriendStatus.Online) {
                ...
                var classType = (Wrath.Class)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
