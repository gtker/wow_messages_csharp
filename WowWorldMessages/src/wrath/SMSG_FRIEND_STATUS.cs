...
public class SMSG_FRIEND_STATUS: WrathServerMessage, IWorldMessage {
    public required Wrath.FriendResult Result { get; set; }
    ...
    public static async Task<SMSG_FRIEND_STATUS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var result = (Wrath.FriendResult)await r.ReadByte(cancellationToken).ConfigureAwait(false);
