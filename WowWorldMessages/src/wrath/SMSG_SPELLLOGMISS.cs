...
public class SMSG_SPELLLOGMISS: WrathServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_SPELLLOGMISS> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfTargets; ++i) {
            targets.Add(await Wrath.SpellLogMiss.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
