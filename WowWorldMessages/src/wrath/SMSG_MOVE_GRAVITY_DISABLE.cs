...
public class SMSG_MOVE_GRAVITY_DISABLE: WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 1230, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_MOVE_GRAVITY_DISABLE> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new SMSG_MOVE_GRAVITY_DISABLE {
            Unit = unit,
