...
public class SMSG_GUILD_INFO: WrathServerMessage, IWorldMessage {
    public required string GuildName { get; set; }
    public required uint Created { get; set; }
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        await w.WriteUInt(Created, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_GUILD_INFO> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var created = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
    ...
    internal int Size() {
        ...
        // created: Generator.Generated.DataTypeDateTime
        size += 4;
