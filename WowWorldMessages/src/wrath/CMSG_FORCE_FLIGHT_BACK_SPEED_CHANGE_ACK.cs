...
public class CMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK: WrathClientMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 900, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<CMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        return new CMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK {
            Player = player,
            Counter = counter,
            Info = info,
            NewSpeed = newSpeed,
