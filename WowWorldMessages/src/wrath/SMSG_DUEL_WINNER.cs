...
public class SMSG_DUEL_WINNER: WrathServerMessage, IWorldMessage {
    public required Wrath.DuelWinnerReason Reason { get; set; }
    ...
    public static async Task<SMSG_DUEL_WINNER> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        var reason = (Wrath.DuelWinnerReason)await r.ReadByte(cancellationToken).ConfigureAwait(false);
