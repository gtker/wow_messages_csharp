...
public class CMSG_BATTLEFIELD_PORT: WrathClientMessage, IWorldMessage {
    /// <summary>
    ...
    public required Wrath.BattlefieldPortAction Action { get; set; }
    ...
    public static async Task<CMSG_BATTLEFIELD_PORT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var action = (Wrath.BattlefieldPortAction)await r.ReadByte(cancellationToken).ConfigureAwait(false);
