...
public class SMSG_ITEM_PUSH_RESULT: WrathServerMessage, IWorldMessage {
    public required ulong Guid { get; set; }
    public required Wrath.NewItemSource Source { get; set; }
    public required Wrath.NewItemCreationType CreationType { get; set; }
    public required Wrath.NewItemChatAlert AlertChat { get; set; }
    ...
    public required uint ItemCountInInventory { get; set; }
    ...
    public async Task WriteBodyAsync(Stream w, CancellationToken cancellationToken = default) {
        ...
        await w.WriteUInt(ItemCountInInventory, cancellationToken).ConfigureAwait(false);

    }

    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, 47, 358, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<SMSG_ITEM_PUSH_RESULT> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        var source = (Wrath.NewItemSource)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var creationType = (Wrath.NewItemCreationType)await r.ReadUInt(cancellationToken).ConfigureAwait(false);

        var alertChat = (Wrath.NewItemChatAlert)await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        var itemCountInInventory = await r.ReadUInt(cancellationToken).ConfigureAwait(false);
        ...
        return new SMSG_ITEM_PUSH_RESULT {
            ...
            ItemCount = itemCount,
            ItemCountInInventory = itemCountInInventory,
        };
    }
