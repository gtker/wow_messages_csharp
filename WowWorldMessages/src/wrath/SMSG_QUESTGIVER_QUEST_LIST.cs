...
public class SMSG_QUESTGIVER_QUEST_LIST: WrathServerMessage, IWorldMessage {
    ...
    public static async Task<SMSG_QUESTGIVER_QUEST_LIST> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
        ...
        for (var i = 0; i < amountOfEntries; ++i) {
            questItems.Add(await Wrath.QuestItem.ReadBodyAsync(r, cancellationToken).ConfigureAwait(false));
        }
