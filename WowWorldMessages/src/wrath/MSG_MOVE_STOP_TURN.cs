...
public class MSG_MOVE_STOP_TURN: WrathClientMessage, WrathServerMessage, IWorldMessage {
    ...
    public async Task WriteEncryptedClientAsync(Stream w, IClientEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteClientHeaderAsync(w, (uint)Size() + 4, 190, cancellationToken).ConfigureAwait(false);
    ...
    public async Task WriteEncryptedServerAsync(Stream w, IServerEncrypter encrypter, CancellationToken cancellationToken = default) {
        await encrypter.WriteServerHeaderAsync(w, (uint)Size() + 2, 190, cancellationToken).ConfigureAwait(false);
    ...
    public static async Task<MSG_MOVE_STOP_TURN> ReadBodyAsync(Stream r, CancellationToken cancellationToken = default) {
